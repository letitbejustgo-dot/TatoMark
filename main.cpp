...
// 显示名清理：若名称里数字前含中文，则截到数字前(去掉如"2016修订版"版本后缀)，让下拉更干净
...
// 字体下拉：面板与每行矩形。行 0..n-1=字体，n=安装，n+1=卸载。返回总行数
...
// 从 PNG 生成 HCURSOR：缩放到光标尺寸，并给形状加白色描边(halo)，保证在任意背景都清晰(参考系统光标白描边)
static HCURSOR MakeCursorFromPng(const std::wstring& path,int size){
    ...
    // 热点：形状最顶端(再最左)像素 → 箭尖(以 hi 坐标找，再折算)
    ...
    case WM_LBUTTONDOWN:{
        ...
        if(g_mode==SEL){ g_start=g_cursor; g_selecting=true; g_sel={x,y,x,y}; SetCapture(hwnd); }
            ...
            if(g_tool==5 && g_fontMenu){ RECT panel; std::vector<RECT> rows; int tot=FontMenuLayout(panel,rows); int nf=(int)g_fontNames.size(); bool hit=false;
                ...
                if(!hit) g_fontMenu=false;   // 点菜单外(含字体框本身)即收起
                InvalidateRect(hwnd,nullptr,FALSE); return 0; }
    ...
    case WM_SETCURSOR:
        if(LOWORD(lParam)==HTCLIENT){
            ...
            if(g_mode==EDIT){
                ...
                else if(g_selRz>=0||g_selMv) c=IDC_ARROW;              // 调整/移动选区 → 常规指针
                else if(g_tool==-1) c=IDC_ARROW;                       // 未选工具(调整阶段) → 常规指针
